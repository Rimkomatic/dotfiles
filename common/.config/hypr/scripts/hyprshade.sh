...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ]; then
        echo "hyprshade_filter=\"$choice\"" >~/.config/custom/settings/hyprshade.sh
        if [ "$choice" == "off" ]; then
            hyprshade off
    ...
    if [ -f ~/.config/custom/settings/hyprshade.sh ]; then
        source ~/.config/custom/settings/hyprshade.sh
    fi
